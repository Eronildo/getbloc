...
export const newBinding = async (uri: Uri) => {
  ...
  const pascalCaseModuleName = changeCase.pascalCase(moduleName.toLowerCase());
  try {
    ...
    const outputChannel = window.createOutputChannel('GetBloc');
    outputChannel.appendLine('Copy and paste this line inside the getPages in GetMaterialApp:');
    outputChannel.appendLine('');
    outputChannel.appendLine(`GetPage(name: ${pascalCaseModuleName}Page.routeName, page: () => ${pascalCaseModuleName}Page(), binding: ${pascalCaseModuleName}Binding(),),`);
    outputChannel.show();
  } catch (error) {
...
async function generateModuleCode(
  ...
  await Promise.all([
    ...
    createBindingTemplate(moduleName, moduleDirectoryPath),
  ]);
}
...
function createPageTemplate(
  ...
  return new Promise(async (resolve, reject) => {
    writeFile(targetPath, getGetViewTemplate(pageName), "utf8", (error) => {
      if (error) {
        reject(error);
        return;
      }
...
function createBindingTemplate(
  bindingName: string,
  targetDirectory: string
) {
  const snakeCaseBindingName = changeCase.snakeCase(bindingName.toLowerCase());
  const targetPath = `${targetDirectory}/${snakeCaseBindingName}_binding.dart`;
  if (existsSync(targetPath)) {
    throw Error(`${snakeCaseBindingName}_binding.dart already exists`);
  }
  return new Promise(async (resolve, reject) => {
    writeFile(targetPath, getBindingTemplate(bindingName), "utf8", (error) => {
      if (error) {
        reject(error);
        return;
      }
